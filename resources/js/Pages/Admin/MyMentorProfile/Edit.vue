                    ...
                    >
                        <header>
                            <h2 class="text-lg font-medium text-gray-900">
                                My Mentor Profile Information
                            </h2>

                            <p class="mt-1 text-sm text-gray-600">
                                Update my mentor profile's name, email, phone number,
...
export default {
    ...
    methods: {
        ...
        onSubmit() {
            ...
            axios
                .post(
                    route("my-mentor-profile.update", this.formData.id),
                    formData,
                    config
                )
                .then((res) => {
                    if (res.data.success) Inertia.visit(route("my-mentor-profile.index"));
                })
                .catch((err) => {
        ...
        goBack() {
            if (this.isTriggeredFromTable) Inertia.visit(route("my-mentor-profile.index"));
            else Inertia.get(route("my-mentor-profile.show", this.formData.id));
        },
