<template>
    <MenuSideBar>
        ...
        <Dialog
            v-model:visible="deleteMentorsDialog"
            :style="{ width: '450px' }"
            header="Confirm"
            :modal="true"
        >
            ...
            <template #footer>
                <Button
                    ...
                    icon="pi pi-times"
                    class="p-button-text"
                    @click="deleteMentorsDialog = false"
                />
                <Button
                    label="Yes"
                    icon="pi pi-check"
...
export default {
    ...
    data() {
        return {
            apiUrl: route("mentor.getData"),
            searchableCols: ["name", "email", "phone_number"],
            statuses: ["ACT", "DSBL"],
            filters: null,
            deleteMentorsDialog: false,
            selectedMentors: null,
            isDeleteSingleMentor: false,
            deleteId: null,
        };
    },
    props: {
    ...
    methods: {
        addNew() {
            Inertia.get(route("mentor.create"));
        },
        show(id) {
            Inertia.get(route("mentor.show", id));
        },
        ...
        confirmMultipleDeleteSelected() {
            this.deleteMentorsDialog = true;
        },
        confirmSingleDeleteSelected(id) {
            this.deleteMentorsDialog = true;
            this.isDeleteSingleMentor = true;
            this.deleteId = id;
        },
        deleteMentors() {
            if (this.isDeleteSingleMentor)
                axios
                ...
                axios
                    .delete(
                        route("mentor.deleteMultipleRecord", {
                            mentorList: this.selectedMentors,
                        })
                    )
                    .then((res) => {
                        if (res.data.success)
            ...
            this.deleteId = null;
            this.selectedMentors = null;
            this.isDeleteSingleMentor = false;
            this.deleteMentorsDialog = false;
        },
        deleteBtnStatus(val) {
            this.selectedMentors = val;
        },
    },
